...
* Install the **probe-rs-debugger** extension in VS Code, by downloading the latest `probe-rs-debugger-0.3.3.vsix` from the [Releases](https://github.com/probe-rs/vscode/releases) page in our repo (we will publish to the Microsoft Extension marketplace when we exit the 'Alpha' phase of this project)
  * Install the extension by running `code --install-extension probe-rs-debugger-0.3.3.vsix` in the terminal
  * Alternatively, 

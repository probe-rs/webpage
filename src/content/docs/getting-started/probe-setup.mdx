...
2. If you want to use probe-rs in SSH session, please see the comment at the end of udev rule and
   and modify it according to it.
3. Run `udevadm control --reload` to ensure the new rules are used.
4. Run `udevadm trigger` to ensure the new rules are applied to already added
   devices.

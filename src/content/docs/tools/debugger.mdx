...
Dape is a well-written Emacs package for DAP debuggers. Setup with Eglot tends to be a little tricky at times, as Dape does not support launch.json configurations, so transposition into the configuration syntax is required. Additionally, in order to support RTT logging, some event handling methods are required. Once setup, however, it works well.

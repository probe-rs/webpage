...
**probe-rs** implements two different debugging protocols, **gdb server** and **dap server**. If you want to debug with VSCode, read [this section](#vscode). Otherwise, if you want to debug in your terminal, read the [section about gdb-server](#gdb-server) is for you. 
...
Dape is a well-written Emacs package for DAP debuggers. Setup with Dape tends to be a little tricky at times, as Dape does not support launch.json configurations, so transposition into the configuration syntax is required. Additionally, in order to support RTT logging, some event handling methods are required. Once setup, however, it works well.

[You can read more about Dape here](https://github.com/svaante/dape/). In addition to the documentation and regular help functions built into Emacs, such as `describe-function`, `describe-variable` and `customize`, you can also visit the [Dape wiki page](https://github.com/svaante/dape/wiki) for more examples for many different DAP debuggers.

Assuming dape is already installed, you can setup dape for probe-rs as follows:
...
## GDB Server
To launch a **gdb server**, simply run
```sh
probe-rs gdb --chip <chip-name>
```

You can then connect to the **gdb-server** by running
```sh
$ gdb
$ (gdb) target remote :1337
```

Alternatively, you can tell **probe-rs** to launch `gdb` for you automatically (the example assumes that you have `gdb` in your path):
```sh
probe-rs gdb --chip <chip-name> --gdb gdb <path-to-elf-file-to-debug>
```

There are also various GUI frontends for GDB that can make your life easier, there's an exhaustive list of them at the [GDB Wiki](https://sourceware.org/gdb/wiki/GDB%20Front%20Ends).

### Working with GDB
**probe-rs** supports most of GDB's features, such as restarting the program, settings breakpoints, and stepping through the program.

For a tutorial on using **gdb**, please consult [their official documentation](https://sourceware.org/gdb/current/onlinedocs/gdb) or search for "gdb cheat sheet" on the web to get started more quickly.
